...
class EmbeddingDataset(Dataset):
    ...
    def __init__(
        ...
        assert dataset_path.suffix == ".csv", "EmbeddingDataset only supports .csv files."
    ...
    def pad_batch(self, batch: list[dict]) -> dict:
        ...
        if self.reference_embedding_dim is not None:
            if self.use_attention:
                reference_embeddings_padded = torch.zeros((batch_size, self.reference_embedding_dim, max_length, max_length), dtype=torch.bfloat16)
            else:
                reference_embeddings_padded = torch.zeros((batch_size, max_length, self.reference_embedding_dim), dtype=torch.bfloat16)
        ...
        for k in range(batch_size):
            ...
            if self.reference_embedding_dim is not None:
                if self.use_attention:
                    reference_embeddings_padded[k, :, :lengths[k], :lengths[k]] = reference_embeddings[k]
                else:
                    reference_embeddings_padded[k, :lengths[k], :] = reference_embeddings[k]
...
def create_dataloader(config: MainConfig, pretrain_config: PretrainMainConfig, split: str) -> torch.utils.data.DataLoader:
    """
    データローダーの作成関数
    Args:
    ...
    if split == "train" or split == "reference":
        dataset_file = pretrain_config.dataset.train_file
    elif split == "validation":
        dataset_file = pretrain_config.dataset.validation_file
    else:
        dataset_file = pretrain_config.dataset.test_file
