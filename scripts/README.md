...
1. 事前学習モデルを用いる場合（data2vec、最終 checkpoint、simple アーキテクチャ）
...
- model: ModelConfig / KnotFoldConfig
  ...
  - KnotFold 特有 (KnotFoldConfig):
    - kf_lambda (float): KnotFold の min-cost-flow アルゴリズムで用いるデフォルトの λ（test 側で kf_lambda_cfg を指定しない場合に使用）。
...
- experiment: ExperimentConfig
  ...
  - kf_lambda_cfg (KfLambdaConfig): KnotFold の min-cost-flow アルゴリズムで用いる λ のスキャン設定。
    ...
    - 3 つすべてが None の場合は、train_config 側で設定された単一の kf_lambda をそのまま用いる。
